-- Demo Mode: every new organization can be seeded with a full, realistic
-- walkthrough (customers, vehicles, leads, appointments, work orders, quotes,
-- invoices, parts) so a first-time user immediately sees ROAVAA's potential
-- instead of an empty app. Seeded rows are tagged is_demo so they can be
-- bulk-deleted later, in the user's own organization (never a shared demo
-- org — that would break the per-org RLS isolation model).
...
alter table public.work_orders add column is_demo boolean not null default false;
...
alter table public.parts add column is_demo boolean not null default false;

create index customers_org_demo_idx on public.customers(organization_id) where is_demo;
create index vehicles_org_demo_idx on public.vehicles(organization_id) where is_demo;
create index work_orders_org_demo_idx on public.work_orders(organization_id) where is_demo;

-- ---------------------------------------------------------------------------
-- seed_demo_data: populates the caller's own organization with a realistic
-- garage snapshot spanning the last ~7 weeks (so reports/graphs aren't flat).
-- Only owner/admin of the org can trigger it (same gate as other bulk writes).
-- ---------------------------------------------------------------------------
...
begin
  ...
  insert into public.leads (organization_id, customer_id, vehicle_id, channel, status, urgency, category, description, is_demo, created_at, updated_at)
  values (p_organization_id, c5, v7, 'phone', 'qualifying', 'normal', 'Entretien', 'Demande de devis pour entretien 15 000 km.', true, now() - interval '3 days', now() - interval '2 days');
...
-- ---------------------------------------------------------------------------
-- delete_demo_data: wipes every is_demo row for this organization, in FK-safe
-- order. Also removes AI-generated rows that reference demo vehicles/work
-- orders even though those child tables have no is_demo column of their own
-- (they're only ever created by trying out a feature on demo data).
-- ---------------------------------------------------------------------------
