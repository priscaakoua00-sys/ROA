...
| Messagerie — WhatsApp/SMS/téléphone | Aucune intégration API ; seul un lien manuel `wa.me` existe ; la copie marketing (page publique + `robinIntro` inutilisé) laissait croire à une prise en charge automatique | Élevée | Copie corrigée : suppression des mentions « WhatsApp » et « notification automatique » dans la simulation de la page publique (3 langues) et dans `robinIntro` | `npx vitest run` (parité i18n) | Corrigé (copie) ; l'absence d'intégration reste réelle et assumée | Non (assumé, pas caché) | Construire une vraie intégration WhatsApp Business si le canal est choisi | messages/{nl,en,fr}.json, src/components/landing/content.ts |
