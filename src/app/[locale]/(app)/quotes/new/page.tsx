...
export default async function NewQuotePage({
      ...
      return (
        <div className="container max-w-lg py-10">
          <div className="flex items-center gap-3">
            <Link href="/quotes/new" className="text-sm text-muted-foreground hover:underline">
          ...
          <h1 className="mt-3 text-2xl font-semibold tracking-tight">
            {t('quotes.forCustomer', { name: name(customer) })}
          </h1>
          {error ? <p className="mt-3 text-sm text-destructive">{t('quotes.error')}</p> : null}

          <form action={createQuoteAction} className="mt-5 space-y-3 rounded-xl border border-border bg-card p-5 shadow-soft">
            ...
            <label className="block space-y-1.5 text-sm">
              ...
              <select name="kind" defaultValue="other" className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm outline-none transition focus-visible:ring-2 focus-visible:ring-ring">
            ...
            <Field label={t('quotes.validUntil')} name="validUntil" type="date" />
            <Field label={t('invoices.notes')} name="notes" />
            <div className="flex items-center gap-3">
              <SubmitButton className="flex-1">{t('quotes.save')}</SubmitButton>
              <Link href="/quotes">
  ...
  return (
    <div className="container max-w-lg py-10">
      ...
      <h1 className="mt-3 text-2xl font-semibold tracking-tight">{t('quotes.newTitle')}</h1>
      <p className="mt-1 text-sm text-muted-foreground">{t('newVehicle.pickCustomer')}</p>

      <form className="mt-4" action={`/${locale}/quotes/new`} method="get">
        <input
          name="q"
          defaultValue={q ?? ''}
