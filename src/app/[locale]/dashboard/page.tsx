...
export default async function DashboardPage({
  ...
  const { data: orgs } = await supabase.from('organizations').select('id, name, slug, default_language').limit(1);
  const org = orgs?.[0];
  ...
  const metrics: { label: string; value: string | number; bar: string }[] = [
    { label: t('dashboard.metricCustomersServed'), value: customersServedToday, bar: 'bg-primary' },
    { label: t('dashboard.metricMessagesAnswered'), value: repliesToday.count ?? 0, bar: 'bg-primary' },
    { label: t('dashboard.metricAppointmentsBooked'), value: apptsToday.count ?? 0, bar: 'bg-gold' },
    { label: t('dashboard.metricFollowupsDone'), value: handledToday.count ?? 0, bar: 'bg-success' },
    { label: t('dashboard.metricTimeSaved'), value: t('dashboard.minutesShort', { count: timeSavedMinutes }), bar: 'bg-success' },
    { label: t('dashboard.metricResponseRate'), value: `${responseRate}%`, bar: 'bg-gold' },
  ];
      ...
      <div className="container py-8">
        <div className="flex flex-wrap items-center justify-between gap-3 animate-in fade-in duration-500">
          <div className="flex items-center gap-2.5">
            <span className="flex size-8 shrink-0 items-center justify-center rounded-[10px] bg-gradient-to-br from-primary to-gold font-serif text-base font-semibold text-primary-foreground shadow-soft">
              {org.name.charAt(0).toUpperCase()}
            </span>
            <h2 className="font-serif text-base font-semibold tracking-tight">{org.name}</h2>
          </div>
        ...
        <section className="mt-5 rounded-2xl border border-gold/25 bg-gradient-to-br from-gold/10 to-transparent p-6 shadow-soft animate-in fade-in slide-in-from-top-2 duration-500">
          ...
          <h1 className="mt-2 font-serif text-2xl font-semibold tracking-tight">{greeting}</h1>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex flex-wrap items-center gap-1.5 truncate text-xs text-muted-foreground">
                        {v.vehicles?.license_plate ? <PlateBadge plate={v.vehicles.license_plate} country={plateCountry} /> : null}
                        <span className="truncate">{fullName(v.customers)}</span>
                      </div>
                      {nextAppt ? (
        ...
        <section className="mt-8 animate-in fade-in duration-700">
          ...
          {metricsAllZero ? (
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-3">
              {metrics.map((s) => (
                <div key={s.label} className="relative overflow-hidden rounded-xl border border-border bg-card p-4 pl-5 shadow-soft transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md">
                  <span className={`absolute inset-y-0 left-0 w-1 ${s.bar}`} aria-hidden />
                  <div className="font-serif text-2xl font-semibold tracking-tight">{s.value}</div>
