...
export default async function VehicleDetailPage({
  ...
  const { data: orgs } = await supabase.from('organizations').select('default_language').limit(1);
  const plateCountry = countryForLanguage(orgs?.[0]?.default_language);
  ...
  return (
    <div className="container max-w-2xl py-10">
      ...
      <h1 className="mt-3 font-serif text-2xl font-semibold tracking-tight">{title}</h1>
